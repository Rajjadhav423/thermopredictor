...
export default function IntelligentSeawaterAnalyzer() {
  ...
  function formatValueToThreeDecimals(value: number | string): string | number {
    if (typeof value === 'number') {
      return value.toFixed(3);
    }
    return value; // agar number nahi hai to original value return kar do
  }
  // Handle seawater prediction
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-start flex-1 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-white text-sm sm:text-base lg:text-lg font-semibold break-words">{formatValueToThreeDecimals(value)}</p>
                          </div>
                        </div>
